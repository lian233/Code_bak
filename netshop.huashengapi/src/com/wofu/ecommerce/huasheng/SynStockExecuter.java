...
public class SynStockExecuter extends Executer {
	...
	private static String jobName="跨境商城(花生API)更新库存作业";
	...
	public void run() {
		...
		try {
			...
			if(Params.tradecontactid.equals("") ||
			   ...
			   Params.vcode.equals(""))
					throw new Exception("缺少调用参数,启动失败!");
	...
	private void synStock() throws Exception
	{
		//店铺同步比例
		double synrate=1;
		Log.info(Params.username,"开始同步商品库存");
			...
			try 
			{
				...
				for(int i=0;i<vtstockconfig.size();i++)
				{
					try{
						...
						{
							Log.info(Params.username,"配置不需要同步库存,货号:"+stockconfig.getItemcode());
							continue;  //不需要同步
						}
						...
						for(int j=0;j<vtstockconfigsku.size();j++)
						{
							try{
								...
								Log.info(Params.username,"SKU:"+stockconfigsku.getSku()+" 原库存:"+stockconfigsku.getStockcount());
								...
								if (this.getDao().intSelect(sql)==0)
								{
									...
									if (this.getDao().intSelect(sql)==0)
									{
										Log.warn(Params.username,"找不到SKU【"+stockconfigsku.getSku()+"】对应的条码,商品标题:"+stockconfig.getTitle());		
