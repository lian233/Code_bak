...
public class OrderDelivery extends Thread {
	...
	private static String jobname = "优购网订单发货处理作业";
	...
	private void delivery(Connection conn)  throws Exception
	{
		...
		for (int i = 0; i < vdeliveryorder.size(); i++) {
	        ...
	        //系统级参数设置
			deliveryParams.put("app_key", Params.app_key);
			deliveryParams.put("format", Params.format);
			deliveryParams.put("method", "yougou.order.completed.update");
			deliveryParams.put("sign_method", "md5");
			deliveryParams.put("app_version", Params.ver);
			deliveryParams.put("timestamp", Formatter.format(new Date(), Formatter.DATE_TIME_FORMAT));
			deliveryParams.put("order_sub_nos", orderid);//订单号
			deliveryParams.put("express_codes", post_no);//运单号
			deliveryParams.put("ship_times", Formatter.format(new Date(), Formatter.DATE_TIME_FORMAT));//发货时间
			deliveryParams.put("logistics_company_codes", postcompanyid);//物流公司
	        
			String responseOrderListData = Utils.sendByPost(deliveryParams, Params.app_secret, Params.url);
			//Log.info("responseOrderListData: "+responseOrderListData);
	        
			JSONObject responseDelivery=new JSONObject(responseOrderListData).getJSONObject("yougou_order_completed_update_response");
					
			String errorCode=responseDelivery.getString("code");
			
			if (!"200".equals(errorCode))
			{
				String errdesc=responseDelivery.getString("message");
				
				Log.warn("订单发货失败,订单号:["+orderid+"],快递公司:["+post_company+"],快递单号:["+post_no+"] 错误信息:"+errdesc);
				/**
				if (errdesc.indexOf("状态异常")>=0)
				{
