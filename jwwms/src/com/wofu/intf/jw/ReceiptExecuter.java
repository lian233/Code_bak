...
public class ReceiptExecuter extends Executer {
	private static String jobName="定时处理巨沃物流出库数据";
	public void run() {
		...
		try {			 
			...
			String sql="select top 5000 msgid,servicetype,bizdata "
				+"from jwinterfaceinfo with(nolock) where flag=0 order by receivedate";
			...
			for (int i=0;i<vtintf.size();i++)
			{
				...
				JWProcess processor = (JWProcess) Class.forName(processorClassName).newInstance();
				...
				sql="update jwinterfaceinfo set flag=1,processdate=getdate() where msgid='"+msgid+"'";
				dao.execute(sql);
