...
public class GenCustomerOrder extends Thread{
	private static String jobName = "麦斯卡订单生成客户订单作业";
	...
	public void run() {
		...
		do {
			...
			try 
			{
				connection = PoolHelper.getInstance().getConnection(com.wofu.ecommerce.maisika.Params.dbname);	
				//新订单
			...
			while (System.currentTimeMillis() - startwaittime < (long) (com.wofu.ecommerce.maisika.Params.waittime * 1000))
				try {
					sleep(1000L);
				} catch (Exception e) {
		...
		{	
			...
			for (Iterator it = slist.iterator(); it.hasNext();) {
				try{
					...
					String result="";
					sql="select HasGoodReturn from ns_Refund where sheetid='"+sheetid+"'";
					result =SQLHelper.strSelect(connection, sql);
					if(result.equals("0")){
						sql="declare @ret int;  execute  @ret = eco_GetRefund '"+sheetid+",1,1';select @ret ret;";
						System.out.println("退款单调用成功");
					}
					
					else{
						sql="declare @ret int;  execute  @ret = eco_GetRefund '"+sheetid+",1,0';select @ret ret;";
						System.out.println("退货单调用成功");
					}
					try
					{
						connection.setAutoCommit(false);
						...
						if (ret==0)
						{
							IntfUtils.backupDownNote(connection, "yongjun",sheetid, "2");
