...
public class SynStockExecuter extends Executer {
	...
	private String app_sercert="";

	private String username="";
	
	private static String jobName="定时更新麦斯卡库存222";
	...
	private void synStock() throws Exception
	{
		//店铺同步比例
		double synrate=1;
		Log.info(username,"开始同步12商品库存");
		
		StringBuffer updateItemsXML=new StringBuffer();
		
		String sql="select orgid from ecs_tradecontactorgcontrast with(nolock) where tradecontactid="+tradecontactid;//120
			...
			try 
			{
				...
				for(int i=0;i<vtstockconfig.size();i++)
				{
					try{
						...
						if (stockconfig.getIsneedsyn()==0)//是否需要同步库存
						{
						...
						for(int j=0;j<vtstockconfigsku.size();j++)
						{
							try{
								...
								qty = Double.valueOf(Math.floor(qty*synrate)).intValue();				
									//要添加app_key,app_sercert
								//StockUtils.updateStock(url, "", "", app_key, "", "", "", "", 0, stockconfigsku.getSku(),  stockconfigsku.getStockcount(),qty , 0, app_sercert);
								StockUtils.updateStock(url, "", "", app_key, "", "", "", "", 0, stockconfigsku.getSku(), stockconfigsku.getStockcount() , qty , 0, app_sercert);
							}catch(Exception ex){
								if (this.getConnection() != null && !this.getConnection().getAutoCommit())
									this.getConnection().rollback();
				...
				if (!outerstocklist.equals(""))
				{
					...
					//StockUtils.updateStock(Params.url, "", "", "", "", "", "", "", 0, sku, addstockqty, qty, 0, "");
