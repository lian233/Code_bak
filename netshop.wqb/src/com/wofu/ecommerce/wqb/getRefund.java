...
public class getRefund extends Thread {
	...
	private static String jobName = "获取网渠宝退货作业";
	...
	public void run() {
		...
		do {		
			...
			while (System.currentTimeMillis() - startwaittime < (long) (com.wofu.ecommerce.wqb.Params.waittime * 1000*5))		
				try {
					sleep(1000L);
				} catch (Exception e) {
		...
		for(int k=0;k<10;)
		{
			try
			{
				while(true)
				{
					...
					refundlistparams.put("user", Params.app_key);
					refundlistparams.put("format", Params.format);
					refundlistparams.put("method", "IOpenAPI.GetExchangeOrderProduct");
					refundlistparams.put("pageRows", "50");
					refundlistparams.put("exchangeStatus", "10");//等待退货
					refundlistparams.put("pageIndex", String.valueOf(pageno));//等待退货
					refundlistparams.put("pageSiz", Params.pageSize);//等待退货
			        ...
			        String responseOrderListData = Utils.sendByPost(refundlistparams, Params.app_secret,"IOpenAPI.GetExchangeOrderProduct", Params.url);
					...
					for(int j=0;j<refundlist.length();j++)
					{
						try{
							...
							if (!OrderManager.RefundIntfExists("检查网渠宝退货单", conn, r.getOrderCode(),r.getRefundCode()))
							{
