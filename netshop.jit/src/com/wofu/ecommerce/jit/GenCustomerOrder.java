...
public class GenCustomerOrder extends Thread{
	private static String jobname = "唯品会JIT订单生成客户订单作业";
		...
		do {		
			...
			try {
				...
				for (int i=0;i<vts.size();i++) {
					...
					// purchaseFlag=9   散单销售
					boolean is_success=OrderManager.genCustomerOrder(connection, sheetid,0,"9",0);				
					if (is_success)
					{
						//备份接口数据
			...
			while (System.currentTimeMillis() - startwaittime < (long) (com.wofu.ecommerce.jit.Params.waittime * 1000))
				try {
					sleep(1000L);
				} catch (Exception e) {
