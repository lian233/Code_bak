...
public class CheckOrderExecuter extends Executer {
	...
	private static String jobName="定时检查百世物流出库单";
		...
		for (int k=0;k<sheetlist.size();k++)
		{
			...
			for(int n=0;n<10;)
			{
				try {
					...
					String msgId=BestUtil.makeMsgId(this.getDao().getConnection());
					...
					else
					{
						...
						if (orderStatus.equalsIgnoreCase("DELIVERED"))		//已发货
						{
							...
							IntfUtils.upNote(this.getDao().getConnection(),owner, commsheetid, 2209, interfacesystem, BestUtil.getShopID(this.getDao().getConnection(),customercode,warehousecode));
						...
						else if (orderStatus.equalsIgnoreCase("CANCELED") || orderStatus.equalsIgnoreCase("CLOSED"))		//取消、关闭
						{
							...
							IntfUtils.upNote(this.getDao().getConnection(), owner, commsheetid, 2209,interfacesystem,
									BestUtil.getShopID(this.getDao().getConnection(),customercode,warehousecode));
