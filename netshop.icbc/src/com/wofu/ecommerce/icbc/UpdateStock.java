					...
					if(vtstockconfigsku.size()>0){     //存在子商品
						for (int j=0;j<vtstockconfigsku.size();j++)
						{
							try{
								...
								StockUtils.updateSkuStock(jobName,dao,orgid,Params.url,"add",stockconfig,stockconfigsku,newqty,false);
							}catch(Exception ex){
						...
						StockUtils.updateItemStock(jobName,dao,orgid,Params.url,"add",stockconfig,newqty);
