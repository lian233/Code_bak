...
public class SynStockExecuter extends Executer {
	...
	private static final String jobName="定时更新美丽说商品库存";
					...
					for(int i=0;i<vtstockconfig.size();i++)
					{
						try{
							...
							if (this.getDao().intSelect(sql)==0)    //如果商品在ecs_stockconfigsku中没有对应的记录，则它会直接在条形码表中有相应的记录，
							{
								...
								StockUtils.updateItemStock(jobName,this.getDao(),orgId,url,"set",stockconfig,qty);
								...
								for(int j=0;j<skuSize;j++)
								{
									try{
										...
										if(j==skuSize-1){   //最后一个sku把主货号的库存也一起更新
											StockUtils.updateSkuStock(jobName,this.getDao(),orgId,url,"set",stockconfig,stockconfigsku,qty,true);
										}else{
											StockUtils.updateSkuStock(jobName,this.getDao(),orgId,url,"set",stockconfig,stockconfigsku,qty,false);
										}
									}catch(Exception ex){
