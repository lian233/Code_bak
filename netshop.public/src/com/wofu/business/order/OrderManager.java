package com.wofu.business.order;
...
public class OrderManager {
	...
	public static boolean genCustomerOrder(Connection conn,String sheetid,int isDelay,int tableType,int isSysPrice) throws Exception
	{	String sql ="";
		sql="declare @ret int; declare @msg varchar(128); "
			+" execute  @ret = eco_genCustomerOrder '"+sheetid+"',@msg output,'',0,'接口',"+isDelay+","+tableType+",1,\"0\",1;"
			+" select @ret ret,@msg msg;";
	...
	//boolean is_success=OrderManager.genCustomerOrder(connection, sheetid, 1, 0,1,"C");
	public static boolean genCustomerOrder(Connection conn,String sheetid,int isDelay,int tableType,int isSysPrice,String what) throws Exception
	{	String sql ="";
		sql="declare @ret int; declare @msg varchar(128); "
			+" execute  @ret = eco_genCustomerOrder '"+sheetid+"',@msg out,'',0,'接口',"+isDelay+","+tableType+",1,'"+what+"',1;"
			+" select @ret ret,@msg msg;";
		/*
		 * exec eco_genCustomerOrder '571N0A1505060012' , @msg out , '',0, '接口',0,0,1,'C',1*/
		System.out.println(sql);
		boolean is_success=false;
		Hashtable ht=SQLHelper.oneRowSelect(conn, sql);
		int ret =Integer.valueOf(ht.get("ret").toString()).intValue();
	...
	//标准的
	//flag: --1:手工重入  2：重建明细
	//LineFlag  0 电商   1：b2b  2:跨境  purchaseFlag(业务类型) '5':电商
	//issysprice  如果是0，则为接口的价格，否则按系统的价格
	//synflag:其它数据是否异步生成 0：即时生成  1：取得客户ID，选择DC,取快递，可用库存，处理赠品等后面处理
	public static boolean genCustomerOrder(Connection conn,String sheetid,int isDelay,int tableType,int LineFlag,String purchaseFlag,int isSystemPrice) throws Exception
	{	String sql ="";
		sql="declare @ret int; declare @msg varchar(128); "
			+" execute  @ret = eco_genCustomerOrder '"+sheetid+"',@msg out,'',0,'接口',"+isDelay+","+tableType+","+LineFlag+",'"+purchaseFlag+"',"+isSystemPrice
			+" select @ret ret,@msg msg;";
		/*
		 * exec eco_genCustomerOrder '571N0A1505060012' , @msg out , '',0, '接口',0,0,1,'C',1*/
		System.out.println(sql);
	...
	public static boolean genEcCustomerOrder(Connection conn,String sheetid) throws Exception
	{	
		String sql="declare @ret int; declare @msg varchar(128); "
				+" execute  @ret = eco_genEcOrder '"+sheetid+"',@msg output;"
				+" select @ret ret,@msg msg;";
	...
	//经销系统用  lineFlag=1代表b2b接口 genCustomerOrder（conn,sheetid,0,'5',1)
	public static boolean genCustomerOrder(Connection conn,String sheetid,int lineFlag,String purchaseFlag,int issysprice) throws Exception
	{	
		String sql="declare @ret int; declare @msg varchar(128); "
				+" execute  @ret = eco_genCustomerOrder '"+sheetid+"',@msg output,'',0,'接口',0,0,"+lineFlag+",'"+purchaseFlag+"',"+issysprice+";"
				+" select @ret ret,@msg msg;";
