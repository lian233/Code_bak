...
public class UpdateStock extends Thread
{
	...
	private static String jobName = "更新美丽说库存作业";
	...
	private void updateStock(Connection conn) throws Throwable
	{

		String sql = "";
		if (orgid == 0)
		{
			sql = "select orgid from ecs_tradecontactorgcontrast with(nolock) where tradecontactid="
		...
		for (int i = 0; i < vtinfo.size(); i++)
		{
			...
			try
			{
				...
				sql = "select * from ecs_stockconfigsku with(nolock) where orgid="
						+ orgid + " and sku='" + sku + "'";

				Vector vtstockconfigsku = SQLHelper.multiRowSelect(conn, sql);
				if (vtstockconfigsku.size() > 0)
				{ // 存在子商品
					for (int j = 0; j < vtstockconfigsku.size(); j++)
					{
						try
						{
							Hashtable htstockconfigsku = (Hashtable) vtstockconfigsku
									.get(j);

							ECS_StockConfigSku stockconfigsku = new ECS_StockConfigSku();
							stockconfigsku.getMapData(htstockconfigsku);

							sql = "select * from ecs_stockconfig with(nolock) where orgid="
									+ orgid
									+ " and itemid='"
									+ stockconfigsku.getItemid() + "'";
							Hashtable htstockconfig = SQLHelper.oneRowSelect(
									conn, sql);
							...
							if ((stockconfigsku.getStockcount() + qty) <= stockconfig
									.getAlarmqty())
							{
							...
							if (newqty < 0)
								newqty = 0;
							Log.info("更新sku库存: ," + "产品编码："
									+ stockconfig.getItemid() + "货号:　"
									+ stockconfig.getItemcode());
							StockUtils.updateSkuStock(Params.token,jobName, dao, orgid,
									Params.url, "add", Params.vcode,
									stockconfig, stockconfigsku, newqty, false,Params.appKey,Params.appsecret);
						} catch (Exception ex)
						{
							if (dao != null
									&& !dao.getConnection().getAutoCommit())
				...
				{ // 没有子商品
					...
					// 如果原库存加上本次需同步的库存小于等于警戒库存,则更新库存为零
					if ((stockconfig.getStockcount() + qty) <= stockconfig
							.getAlarmqty())
					{
					...
					//StockUtils.updateItemStock(jobName, dao, orgid, Params.url,
						//	"add", stockconfig, Params.vcode, newqty);
			...
			} catch (Throwable je)
			{
				if (conn != null && !conn.getAutoCommit())
					conn.rollback();
				// StockManager.bakSynReduceStore(jobName, conn,
				// Params.tradecontactid, tid, sku);
				Log.error(jobName, "更新美丽说库存失败,单号:" + tid + " SKU:" + sku + " "
						+ je.getMessage());
				continue;
			}
		}
		if (vtinfo.size() > 0)
		{
			...
			StringBuilder sqlstr = new StringBuilder()
					.append("update eco_synreducestore set synflag=1 where id in(");
			for (int i = 0; i < vtinfo.size(); i++)
			{
				if (i == vtinfo.size() - 1)
				{
					sqlstr.append(((Hashtable) (vtinfo.get(i))).get("id"))
							.append(")");
				} else
				{
					sqlstr.append(((Hashtable) (vtinfo.get(i))).get("id"))
