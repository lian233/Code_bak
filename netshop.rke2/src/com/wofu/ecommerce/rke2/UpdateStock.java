		...
		for(int i=0;i<vtinfo.size();i++)
		{
			...
			{		
				...
				for (int j=0;j<vtstockconfigsku.size();j++)
				{
					...
					newqty=StockManager.getTradeContactUseableStock(conn, Integer.parseInt(Params.tradecontactid), stockconfigsku.getSku(),true);
					if(newqty<0) newqty=0;
					StockUtils.updateStock(Params.url, Params.ver, stockconfigsku, newqty);
