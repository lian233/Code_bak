...
public class OrderDelivery extends Thread {
	...
	private static String jobname = "一号店订单发货处理作业";
	...
	private void delivery(Connection conn)  throws Exception
	{
		...
		for (int i = 0; i < vdeliveryorder.size(); i++) {
			...
			params.put("method", "yhd.logistics.order.shipments.update");
			...
			params.put("deliverySupplierId",postcompanyid);
			params.put("expressNbr", post_no);
			...
			if (errorCount>0)
			{
				...
				for(int j=0;j<errlist.length();j++)
				{
					...
					errdesc=errdesc+" "+errinfo.getString("errorCode")+" "+errinfo.getString("errorDes"); 
	...
	private void modifiRemark(Connection conn) throws Exception{
		String sql = "select sheetid,sender from it_upnote with(nolock) where sheettype=5 and flag=0 and receiver='"+Params.tradecontactid+"'";
