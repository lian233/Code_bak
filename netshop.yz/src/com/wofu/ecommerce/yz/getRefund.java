...
public class getRefund extends Thread {
	...
	private static String jobName = "获取有赞退货作业";
	...
	public void run() {
		...
		do {		
			...
			while (System.currentTimeMillis() - startwaittime < (long) (com.wofu.ecommerce.yz.Params.waittime * 1000*5))		
				try {
					sleep(1000L);
				} catch (Exception e) {
		...
		for(int k=0;k<10;)
		{
			try
			{
				while(true)
				{
					...
					refundlistparams.put("appKey", Params.app_id);
			        ...
			        String responseOrderListData = Utils.sendByPost(refundlistparams, Params.AppSecret, Params.url);
					...
					for(int j=0;j<refundlist.length();j++)
					{
						try{
					        ...
					        //系统级参数设置
							orderparams.put("appKey", Params.app_id);
							...
							String responseData =Utils.sendByPost(orderparams, Params.AppSecret, Params.url);
							...
							if (!OrderManager.RefundIntfExists("检查有赞退货单", conn, r.getOrderCode(),r.getRefundCode()))
							{
