...
public class SynStockExecuter extends Executer {
	...
	public void run() {
		...
		app_key=prop.getProperty("app_key",Params.app_key);
		app_sercert=prop.getProperty("app_sercert",Params.app_Secret);
					...
					try{
						...
						for(int j=0;j<vtstockconfigsku.size();j++)
						{
							try{
								...
								StockUtils.updateStock(url, stockconfigsku , qty , 0, this.getDao());
							}catch(Exception ex){
								if (this.getConnection() != null && !this.getConnection().getAutoCommit())
									this.getConnection().rollback();
