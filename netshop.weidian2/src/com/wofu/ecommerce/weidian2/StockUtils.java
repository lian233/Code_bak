...
//				
//				Log.info("更新微店成功,更新总SKU数:["+updateCount+"]");
//				
//				updateStockConfig(dao,orgid,sku,errMsg);
//			}
//		} catch (Exception e) {
//			Log.info("更新微店库存失败,错误信息:"+e.getMessage());
...
//	}
	...
	private static void updateStockConfig(DataCentre dao,int orgid,String outerstocklist,int updatetype) throws Exception
	{
		...
		for (int i=0;i<outerstockarr.length;i++)
		{
			...
			if (updatetype==1)
			{
				...
				Log.info("更新微店成功,SKU:"+sku+",新库存:"+qty);
			}
			else
			{
				...
				Log.info("更新微店成功,SKU:"+sku+",调整库存:"+qty);
			}
	...
	{
		//service=stock&vcode=F9FE35AE-A901-47B5-BB71-4F5EE8F75C3C&type=set&skulist=96dc311b-46e5-473f-a363-1d25425d93d8&qtylist=1000
		StringBuffer  buffer = new StringBuffer(); 
		buffer.append("service=stock&");
		buffer.append("vcode="+Params.vcode+"&");
		buffer.append("type=set&");
		buffer.append("skulist="+sku+"&");
		buffer.append("qtylist="+newqty);
		String result = Utils.sendByPost("http://www.royalrose.com.cn/api/Stock/StockDeliver", buffer.toString()); //POST /api/Stock/StockDeliver HTTP/1.1
		boolean flag = false;
		JSONObject responseupdatestock=new JSONObject(result);
...
//			
			String errMsg="";
			String errorDes=response.getString("msg");
