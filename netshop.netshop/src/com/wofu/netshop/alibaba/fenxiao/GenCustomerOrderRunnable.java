...
public class GenCustomerOrderRunnable implements Runnable{
	private String jobName="阿里巴巴接口订单生成客户订单作业";
	...
	public void run() {
		...
		try {
			...
			Log.info(param.username,"本次共要处理的阿里巴巴接口订单生成临时订单总数为: "+vts.size(),null);
			int sheetid=0;
			for (int i=0;i<vts.size();i++) {
				try{
					...
					if (is_success)
					{
						...
						Log.info(param.username,"生成客户订单成功,接口单号【" + sheetid + "】",null);
					}
					else
					{
						try {
						...
						Log.info(param.username,"生成客户订单失败,接口单号【" + sheetid + "】",null);
					}
				}catch(Exception ex){
					try {
						if (connection != null && !connection.getAutoCommit())
					...
					Log.error(param.username,jobName+" "+ex.getMessage(),null);
					Log.info(param.username,"生成客户订单失败,接口单号【" + sheetid + "】,详细信息： "+ex.getMessage(),null);
				}
			...
			Log.error(param.username,jobName+" "+Log.getErrorMessage(e),null);
		} finally {
			try {
				if (connection != null)
					connection.close();
