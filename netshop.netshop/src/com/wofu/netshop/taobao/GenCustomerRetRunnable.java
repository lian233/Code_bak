...
public class GenCustomerRetRunnable implements Runnable{
	...
	public void run() {
		...
		try {
			...
			Log.error(username, jobName+" "+Log.getErrorMessage(e));
		} finally {
			try {
				if (connection != null)
					connection.close();
	...
	{
		Log.info(username,"本次共要处理的淘宝退货单总数为: "+slist.size(),null);
		for (Iterator it = slist.iterator(); it.hasNext();) {
			...
			{		
				Log.info(username,"生成客户退货失败!淘宝单号【"+refsheetid+"】,接口单号【" + sheetid + "】",null);
				Log.info(username,"详细错误信息："+e1.getMessage(),null);
		...
		Log.info(username,jobName+" 执行完毕！",null);
	}
	...
	private List<String> getSheetList(Connection conn)
	{
		...
		{
			Log.error(username,jobName+" 取通知单号出错!",null);
		}
		return sheetlist;
	}
