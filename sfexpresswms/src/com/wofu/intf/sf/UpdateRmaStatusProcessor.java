...
public void process() throws Exception {
		...
		try
		{
			...
			if (rmaStatus.equalsIgnoreCase("FULFILLED"))		//收货完成
			{
				...
				getInStockDetail(commsheetid,updateRmaStatusele);
			...
			else if (rmaStatus.equalsIgnoreCase("CANCELED") || rmaStatus.equalsIgnoreCase("CLOSED"))		//取消
			{
				...
				sfUtil.updateMsg(this.getExtconnection(), rmaCode, 2222, 1);
			else if (rmaStatus.equalsIgnoreCase("WMS_REJECT"))   //接单失败
				sfUtil.updateMsg(this.getExtconnection(), rmaCode, 2222, -1);
			
			
			Log.info("best logistics","接收客户退货单状态成功,单号:"+rmaCode+" 状态:"+rmaStatus);
	...
	private void getInStockDetail(String commsheetid,Element asnele) throws Exception
	{
