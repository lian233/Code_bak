...
public class UpdateStock extends Thread{
	...
	private static String jobName = "更新papago8库存作业";
	...
	private void updateStock(Connection conn) throws Throwable
	{
		...
		for(int i=0;i<vtinfo.size();i++)
		{
					...
					if(vtstockconfigsku.size()>0){     //存在子商品
						for (int j=0;j<vtstockconfigsku.size();j++)
						{
							try{
								...
								StockUtils.updateSkuStock(jobName,dao,orgid,Params.url,Params.Key,stockconfig,stockconfigsku,newqty,false);
							}catch(Exception ex){
						...
						StockUtils.updateItemStock(jobName,dao,orgid,Params.url,stockconfig,Params.Key,newqty);
			...
			{	if(conn!=null && !conn.getAutoCommit())
				...
				Log.error(jobName,"更新papago8库存失败,单号:"+tid+" SKU:" +sku+" "+je.getMessage());
				continue;
			}
		}
		if(vtinfo.size()>0){
