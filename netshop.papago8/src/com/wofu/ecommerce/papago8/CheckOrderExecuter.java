...
public class CheckOrderExecuter extends Executer {
	...
	private static String jobName="定时检查papago8未入订单";
	...
	**/
	public  void checkWaitStockOutOrders() throws Exception
	{
		...
		int pageIndex = 0 ;  //papago8的订单从0页算起
						...
						{	
							Log.info("订单号【"+ o.getTid() +"】,状态【"+ o.getStatus() +"】") ;
							  //正常订单
								...
								//40交易关闭
								else if("交易取消".equals(o.getStatus()))
								{
								...
								else if ("交易成功".equals(o.getStatus()))  //交易成功
								{
									for(Iterator ito=o.getOrderItemList().getRelationData().iterator();ito.hasNext();)
									{
										OrderItem item=(OrderItem) ito.next();
								...
								}else if ("等待付款".equals(o.getStatus()))  //交易成功
								{
									for(Iterator ito=o.getOrderItemList().getRelationData().iterator();ito.hasNext();)
									{
										OrderItem item=(OrderItem) ito.next();
			...
			try 
			{
				...
				while(hasNextPage)
				{
					...
					if(DOMHelper.ElementIsExists(urlset,"Error"))
					{
						...
						if(!"".equals(operCode))
						{
							Log.error("获取papago8订单列表", "获取订单列表失败，操作码："+operCode+",操作结果信息："+operation);
							hasNextPage = false ;
							break ;
						}
					}
