...
public class SynStockExecuter extends Executer {
	...
	private static String jobName="定时同步库存";
				...
				for(int i=0;i<vtstockconfig.size();i++)
				{
					try{
						...
						if (this.getDao().intSelect(sql)==0)    //如果商品在ecs_stockconfigsku中没有对应的记录，则它会直接在条形码表中有相应的记录，
						{
							...
							StockUtils.updateItemStock(this.getDao(),url,appkey,appsecret,authcode,stockconfig,qty,1);
							...
							for(int j=0;j<vtstockconfigsku.size();j++)
							{
								try{
									...
									if(isfind)
									{
										...
										if (qty<0) qty=0;
										StockUtils.updateSkuStock(this.getDao(),url,appkey,appsecret,authcode,stockconfig,stockconfigsku,qty,1);
									}
