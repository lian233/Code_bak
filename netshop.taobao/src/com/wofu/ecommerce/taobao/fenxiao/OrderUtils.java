...
public class OrderUtils {
	...
	public  static void  updateFinishedStatus(Connection conn,int shopid,long tid,Date endtime) throws Exception
	{
		String sql="update DecOrder0 set endtime='"+Formatter.format(endtime, Formatter.DATE_TIME_FORMAT)
			+"' where refsheetid='"+tid+"' and ShopID="+shopid;
		int rowcnt=SQLHelper.executeSQL(conn, sql);
		
		if (rowcnt==0)
		{
			sql="update DecOrder set endtime='"+Formatter.format(endtime, Formatter.DATE_TIME_FORMAT)
				+"' where refsheetid='"+tid+"' and ShopID="+shopid;
			rowcnt=SQLHelper.executeSQL(conn, sql);
			
			if (rowcnt==0)
			{
				sql="update DecOrder0 set endtime='"+Formatter.format(endtime, Formatter.DATE_TIME_FORMAT)
					+"' where sheetid in(select sheetid from MerDecOrderLog where refsheetid='"+tid+"') and ShopID="+shopid;
				rowcnt=SQLHelper.executeSQL(conn, sql);
				if (rowcnt==0)
				{
					sql="update DecOrder set endtime='"+Formatter.format(endtime, Formatter.DATE_TIME_FORMAT)
	...
	public static void getDistributorByNick(Connection conn,String tradecontactid,String nick) throws Exception
	{
		...
		Log.info("size :"+response.getDistributors().size());
