...
public class RDSUtils {
		...
		try {
			...
			if (isFormal)
			{
				...
				if (t.getSeller_memo()!=null && !t.getSeller_memo().equals("null"))
					sellermemo=StringUtil.replace(t.getSeller_memo(),"'","").replaceAll("\n", " ");
				
				if (t.getTrade_memo()!=null && !t.getTrade_memo().equals("null"))
					tradememo=StringUtil.replace(t.getTrade_memo(),"'","").replaceAll("\n", " ");
			...
			String value= dao.strSelect(sql);
			int isremovefirst=Integer.valueOf(!"".equals(value)?value:"0").intValue();
			...
			**/
			...
			sql =new StringBuilder().append("insert into itf_DecOrder")
					...
					.append(" values(")
					.append(sheetid).append(",").append(shopid).append(",'").append(t.getTid())
					.append("','").append(StringUtil.notNullString(t.getSeller_nick())).append("','").append(StringUtil.notNullString(t.getType()))
					.append("','").append(Formatter.format(new Date(), Formatter.DATE_TIME_FORMAT)).append("','")
					.append(Formatter.format(t.getCreated(),Formatter.DATE_TIME_FORMAT)).append("','")
					.append(buyermessage).append("','").append( StringUtil.notNullString(t.getShipping_type()))
					.append("','").append(payment).append("','0.0','").append(adjustfee).append("','").append(returnOrderStatus(StringUtil.notNullString(t.getStatus())))
					.append("','").append(buyermemo).append("','").append(sellermemo).append("','").append(tradememo)
					.append("','").append(Formatter.format(paytime,Formatter.DATE_TIME_FORMAT)).append("','")
					.append(Formatter.format(t.getModified(),Formatter.DATE_TIME_FORMAT)).append("','")
					.append(Formatter.format(t.getModified(),Formatter.DATE_TIME_FORMAT))
					.append("','").append( t.getBuyer_obtain_point_fee()).append("','").append(t.getPoint_fee())
					.append("','").append(t.getReal_point_fee()).append("','").append(t.getTotal_fee()).append("','")
					.append(t.getPost_fee()).append("','").append(StringUtil.notNullString(t.getBuyer_alipay_no()))
					.append("','").append(StringUtil.notNullString(t.getBuyer_nick()).replaceAll("'", " "))
					.append("','").append(StringUtil.notNullString(t.getReceiver_name()).replaceAll("'", " "))
					.append("','").append(StringUtil.notNullString(t.getReceiver_state()))
					.append("','").append(StringUtil.notNullString(t.getReceiver_city()))
					.append("','").append(StringUtil.notNullString(t.getReceiver_district()))
					.append("','").append(StringUtil.notNullString(t.getReceiver_address()).replaceAll("'", " "))
					.append("','").append(StringUtil.notNullString(t.getReceiver_zip()))
					.append("','").append(StringUtil.notNullString(t.getReceiver_mobile()))
					.append("','").append(receiverphone)
					.append("','").append(Formatter.format(t.getModified(),Formatter.DATE_TIME_FORMAT))
					.append("','").append(StringUtil.notNullString(t.getBuyer_email()))
					.append("','").append(String.valueOf(haspostFee))
					.append("','").append(receivedpayment)
					.append("','0','','").append(aplipayno)//cod状态和快递现在留空
					.append("','").append(buyerflag)
					.append("','").append(sellerflag)
					.append("','").append(brandsaleflag).append("','',")//dealRateState订单评价状态暂时为空
					.append("0,'','','")//InvoiceFlag=0   invoicetitle="";  Prepay=''
					.append(Types.convertBooleanToShort(sellerrate))
					.append("','").append(Types.convertBooleanToShort(buyerrate))
					.append("','").append(promotion)
					.append("','").append(StringUtil.notNullString(t.getTrade_from()))
					.append("','").append(alipayurl)
					.append("','").append(promotionDetails)
					.append("','").append(paymode).append("')").toString();
			dao.execute(sql);
			for (Iterator itorder = t.getOrders().getRelationData().iterator(); itorder.hasNext();) {
				...
				int subid = dao.intSelect(sql);
				Log.info("refundstatus: "+o.getRefund_status());
				sql = new StringBuilder("insert into itf_DecOrderItem(ID , ParentID  , skuid , itemmealname , title , ")
						...
						.append(skuid).append("','").append(o.getItem_meal_name()!=null?o.getItem_meal_name():"")
						.append("','").append(o.getTitle().replaceAll("'", " "))
						.append("','").append(o.getSeller_nick()!=null?o.getSeller_nick():"")
						.append("','").append(o.getBuyer_nick()!=null?o.getBuyer_nick():"")
						.append("','").append(o.getSeller_type())
						.append("','").append(Formatter.format(t.getCreated(),Formatter.DATE_TIME_FORMAT))
						//0没有退款。10买家已经申请退款，等待卖家同意。20卖家已经同意退款，等待买家退货。30买家已经退货，等待卖家确认收货。40卖家拒绝退款。90退款关闭。100退款成功。
						.append("',").append(returnrefundStatus(o.getRefund_status()))
						.append(",'").append(o.getOuter_iid())
						.append("','").append(outerskuid.replaceAll("'",""))
						.append("','").append(o.getTotal_fee())
						.append("','").append(o.getPayment())
						.append("','").append(o.getDiscount_fee())
						.append("','").append(o.getAdjust_fee())
						.append("','").append(returnOrderStatus(o.getStatus()))
						.append("','")//timeoutactiontime   订单超时到期时间暂时为空
						.append("','")//iid也为空
						.append("','").append(skuprop)
						.append("',").append(o.getNum())
						.append(",'").append(o.getPrice())
						.append("','").append(o.getPic_path())
						.append("','").append(o.getOid())
						.append("','").append(o.getSnapshot_url())
						.append("','").append(snapshot)
						.append("','").append(Types.convertBooleanToShort(buyerrate))
						.append("','").append(Types.convertBooleanToShort(sellerrate))
						.append("','")//sellertype为空
						.append("',").append(refundid)
						.append(",'").append(Types.convertBooleanToShort(isoversold))
						.append("','")//modified为空
						.append("','").append(o.getNum_iid())
						.append("','").append(cid)
	...
	public static void processOrder(String jobname,DataCentre dao, Trade td,
			int shopid,String sellernick,boolean waitbuyerpayisin, boolean isJzservice,
			boolean finishisin,int customerid,boolean isUpdateStock) throws Exception {
		...
		{	
			...
			//{
				if (!OrderManager.TidLastModifyIntfExists("检查淘宝订单", dao.getConnection(), String.valueOf(td.getTid()),td.getJdp_modified()))
				{
					createOrder(dao,td,shopid,sellernick,true,isJzservice);
				}
				//写入实时更新库存表记录
				if(isUpdateStock)
				for(Iterator it = td.getOrders().getRelationData().iterator();it.hasNext();){
					...
					int qty = o.getNum();
					if("WAIT_SELLER_SEND_GOODS".equals(o.getStatus()))
					StockManager.addSynReduceStore("写入实时同步库存表", dao.getConnection(), shopid, customerid,
							td.getStatus(), String.valueOf(td.getTid()), sku, qty,false);
		...
		{						
			...
			if (waitbuyerpayisin)
			{
				if (!OrderManager.TidLastModifyIntfExists("检查淘宝订单", dao.getConnection(), String.valueOf(td.getTid()),td.getJdp_modified()))
				{
					createOrder(dao,td,shopid,sellernick,false,isJzservice);
		...
		else if (td.getStatus().equals("TRADE_FINISHED"))
		{
			for(Iterator ito=td.getOrders().getRelationData().iterator();ito.hasNext();)
			{
				...
				//StockManager.deleteWaitPayStock(jobname, dao.getConnection(),tradecontactid, String.valueOf(td.getTid()), sku);		
				//更新结束时间
				OrderUtils.updateFinishedStatus(dao.getConnection(),shopid,td.getTid(),td.getEnd_time());
	...
	public static void processItem(String jobname,DataCentre dao, Item item,
			int customerid,int ShopID) throws Exception {
		...
		{	
			...
			map.put("num_iid", item.getNum_iid());
			map.put("sku_id",sku.getSku_id());
			map.put("outerskuid",sku.getOuter_id());
			map.put("num",sku.getQuantity());
			map.put("outerskuid",sku.getOuter_id());
			map.put("title",item.getTitle());
			map.put("modified",sku.getModified());
			map.put("created",sku.getCreated());
			map.put("delist_time",item.getDelist_time());
			map.put("list_time",item.getList_time());
			map.put("itemcode",item.getOuter_id());
			map.put("price",sku.getPrice());
			map.put("approve_status",item.getApprove_status());
			map.put("cid",item.getCid());
			map.put("pic_url",item.getPic_url());
			map.put("properties_name",sku.getProperties_name());
			map.put("props_name",item.getProps_name());
			String[] colorAndSize =getColorAndSize(sku.getProperties_name());
			map.put("colorName", colorAndSize[0]);
			map.put("sizeName", colorAndSize[1]);
			StockManager.addDecItem(jobname,dao, map) ;
			map.clear();
			
		}

	}
	//1627207:28320:颜色分类:白色;20509:28314:尺码:S
	private static String[] getColorAndSize(String properties_name) {
		String[] colorAndSize = new String[2];
		String[] temp = properties_name.split(";");
		for(String e:temp){
			if(e.indexOf("颜色")>0){
				colorAndSize[0]=e.split(":")[3];
			}else if(e.indexOf("尺码")>0){
				colorAndSize[1]=e.split(":")[3];
			}
		}
		return colorAndSize;
	}
	...
	private static int returnOrderStatus(String status){
	...
	//0没有退款。10买家已经申请退款，等待卖家同意。20卖家已经同意退款，等待买家退货。30买家已经退货，等待卖家确认收货。40卖家拒绝退款。90退款关闭。100退款成功。
	private static int returnrefundStatus(String status){
		if("NO_REFUND".equals(status))
			return 0;
		else if("WAIT_SELLER_AGREE".equals(status))
			return 10;
		else if("TRADE_FINISHED".equals(status))
			return 100;
		else if("WAIT_BUYER_CONFIRM_GOODS".equals(status))
