...
public class genCustomerRet extends Thread{
	private static String jobname = "淘宝订单生成客户退货作业";
	...
	private List<String> getSheetList(Connection conn)
	{
		...
		try
		{
			sql="select top 500 sheetid from it_downnote with(nolock) where sender='"+Params.tradecontactid+"' and sheettype=2";
	...
	{
		Log.info("本次共要处理的淘宝退货单总数为: "+slist.size());
...
/*					if (err==-1)    //异常被捕获，这里没有运行的机会
					{							
						//throw new JException("备份数据失败!"+sql);
						conn.rollback();
						Log.info("生成客户退货,执行过程IP_DownBak备份原始数据失败!淘宝单号【"+refsheetid+"】,接口单号【" + sheetid + "】");
						continue;
					}*/
				...
				/*else if (ret==-1)   //异常被捕获，这里没有运行的机会
				{
					Log.info("生成客户退货失败!，执行过程eco_GetRefund生成客户退货单出错，淘宝单号【"+refsheetid+"】,接口单号【" + sheetid + "】");
