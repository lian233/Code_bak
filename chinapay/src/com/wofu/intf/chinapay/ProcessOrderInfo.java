...
public class ProcessOrderInfo extends Thread {
	...
	private static String jobname = "处理订单作业";		
		...
		do {
			...
			try {										
				...
				for(Iterator it=infsheetlist.iterator();it.hasNext();){
					...
					/*
					String sql = "select A.*,"
								+ " (select sum(Price*NotifyQty) Sum from OutStockNoteItem where SheetID = A.SheetID) GoodsFeeSum "
								+ " (select sum(Weigh*NotifyQty)/1000.000 Sum from OutStockItem where SheetID=A.SheetID) GrossWeightSum "
								+ " from OutStockNote A where SheetID = '"+ SheetID+"'";*/
					...
					for (int j=0; j<vtItem.size(); j++) {
						...
						/*
						String spec = htItem.get("SizeName").toString().trim() ;
						if (!"0".equals(htItem.get("ColorID").toString())){
							spec = htItem.get("ColorName").toString().trim() + spec; 
						}
					...
					//Log.info("Country：" + strCountryCode);
					...
					bizSheet.append(DtcTools.CreateItem("BIZ_TYPE_CODE" , "I20" , null));//业务类型  直购进口：I10,网购保税进口：I20
					...
					bizSheet.append(DtcTools.CreateItem("RECEIVER_NAME" , "CertName" , dtOut));//RECEIVER_NAME
					...
					bizSheet.append(DtcTools.CreateItem("TAX_FEE" , 
							sf.format(totalTaxFee.doubleValue()) , null));//税金总额
					else{
						bizSheet.append(DtcTools.CreateItem("TAX_FEE" , 
								"0.0000" , null));//税金总额
					}
					...
					for (int i=0;i<vtsku.size();i++){	
						...
						bizData.append(DtcTools.CreateItem("CUSTOMS_CODE" , Params.customercode , null));
						bizData.append(DtcTools.CreateItem("BIZ_TYPE_CODE" , Params.biz_type_code , null));
						bizData.append(DtcTools.CreateItem("ORIGINAL_ORDER_NO" , "CustomPurSheetID" , htsku));
						...
						bizData.append(DtcTools.CreateItem("DESP_ARRI_COUNTRY_CODE" , Params.DESP_ARRI_COUNTRY_CODE , null));
						bizData.append(DtcTools.CreateItem("RECEIVER_ID_NO" , "CertNo" , htsku));
						bizData.append(DtcTools.CreateItem("RECEIVER_NAME" , "CertName" , htsku));
						bizData.append(DtcTools.CreateItem("RECEIVER_ADDRESS" , "Address" , htsku));
						bizData.append(DtcTools.CreateItem("RECEIVER_TEL" , "Tele" , htsku));
											
						bizData.append(DtcTools.CreateItem("GOODS_FEE" , "GoodsFeeSum", htsku));						
						bizData.append(DtcTools.CreateItem("TAX_FEE" , "Tax" , htsku));											
						bizData.append(DtcTools.CreateItem("GROSS_WEIGHT" , "GrossWeightSum", htsku));
											
						bizData.append("<PROXY_ENT_CODE />");
						bizData.append("<PROXY_ENT_NAME />");

						//订单明细
						bizData.append("<ORDER_DETAIL>");

						sql = "select CustomBC, CustomsListNO, NotifyQty, a.Price, Weigh/1000 Weigh, NetWeigh/1000 NetWeigh, a.Notes, "
								+ " UnitName, a.Price * NotifyQty TotalPrice, IsNull(C.Name, '') + ' ' + IsNull(S.Name, '') Spec "
							  + " from OutStockNoteItem a , Barcode b , Merchandise m , Size s , Color c "
							  + " where a.BarcodeID = b.BarcodeID and a.MID= b.MID and b.MID = m.MID and a.MID = m.MID "
							  + "  and M.MeasureType = s.MeasureTypeID and b.SizeID *= s.SizeID and b.ColorID *= c.ColorID "
							  + " and SheetID = '" + SheetID + "' and CustomsListNO is not null order by a.BarcodeID ";
						
						vtItem = SQLHelper.multiRowSelect(conn, sql);
						for (int j=0; j<vtItem.size(); j++) {
							htItem=(Hashtable) vtItem.get(j);
							bizData.append(DtcTools.CreateItem("SKU" , "CustomBC" , htItem));
							bizData.append(DtcTools.CreateItem("GOODS_SPEC" , "Spec" , htItem));//规格
							bizData.append(DtcTools.CreateItem("CURRENCY_CODE" , Params.CURRENCY_CODE , null));
							bizData.append(DtcTools.CreateItem("PRICE" , "Price" , htItem));
							bizData.append(DtcTools.CreateItem("QTY" , "NotifyQty" , htItem));
							bizData.append(DtcTools.CreateItem("GOODS_FEE" , "TotalPrice" , htItem));
							bizData.append(DtcTools.CreateItem("TAX_FEE" , "0" , null));
						}
						bizData.append("</ORDER_DETAIL>");
					...
					if (bizData.length() > 0){
						...
						bizData.insert(0, DtcTools.createHead(messageType));
