...
public class OrderDelivery extends Thread
{
	...
	public void run() {
		...
		do {
			...
			is_exporting = true;
			try {		
				connection = PoolHelper.getInstance().getConnection(com.wofu.ecommerce.meilishuo2.Params.dbname);
			...
			while (System.currentTimeMillis() - startwaittime < (long) (com.wofu.ecommerce.meilishuo2.Params.waittime * 1000))
				try {
					sleep(1000L);
				} catch (Exception e) {
	...
	private  static boolean delivery(String jobname,Connection conn,Hashtable<String, String> hto)
	{
		...
		try 
		{
			...
			Log.info("access_token ："+object.optString("access_token"));
			 //发送请求
			String responseText = Utils.sendbyget(Params.url, Params.appKey, Params.appsecret, apimethod.trim(), object
					.optString("access_token").trim(), new Date(), orderCode, "", postCompany_trans.toLowerCase(), postNo, "");
