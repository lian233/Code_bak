					...
					for(int i=0;i<vtstockconfig.size();i++)
					{
						try{
							...
							if (this.getDao().intSelect(sql)==0)    //如果商品在ecs_stockconfigsku中没有对应的记录，则它会直接在条形码表中有相应的记录，
							{
								...
								Log.info("同步过去的库存量:  "+qty+"    tradecontactid:   "+tradecontactid);
							}
								...
								for(int j=0;j<skuSize;j++)
								{
									try{
										...
										if(j==skuSize-1){   //最后一个sku把主货号的库存也一起更新
											StockUtils.updateSkuStock(jobName,this.getDao(),orgId,url,"set",vcode,stockconfig,stockconfigsku,qty,true);
										}else{
											StockUtils.updateSkuStock(jobName,this.getDao(),orgId,url,"set",vcode,stockconfig,stockconfigsku,qty,false);
										}
										//Log.info("同步过去的库存量:  "+qty+"    tradecontactid:   "+tradecontactid);
