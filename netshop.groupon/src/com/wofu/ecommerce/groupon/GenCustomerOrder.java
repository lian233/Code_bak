...
public class GenCustomerOrder extends Thread{
	private static String jobname = "团宝订单生成客户订单作业";
		...
		do {		
			...
			try {
				...
				for (int i=0;i<vts.size();i++) {
					...
					if (is_success)
					{
						String sql="select a.sheetid,a.refsheetid from customerorder0 a,ns_customerorder b where a.refsheetid=b.tid and b.sheetid='"+sheetid+"' "
						+"union select a.sheetid,a.refsheetid from customerorder a,ns_customerorder b where a.refsheetid=b.tid and b.sheetid='"+sheetid+"'";
						Hashtable htt=SQLHelper.oneRowSelect(connection, sql);
					
						Log.info("生成客户订单成功,客户订单号【"+htt.get("sheetid").toString()+"】,团宝单号【"+htt.get("refsheetid").toString()+"】,接口单号【" + sheetid + "】");
					}
				}		
			} catch (Exception e) {
				try {
