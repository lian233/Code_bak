...
public class OrderDelivery extends Thread {
	...
	private void doDelivery(Connection conn,Vector<Hashtable> vdeliveryorder) throws SQLException
	{
		...
		for (int i = 0; i < vdeliveryorder.size(); i++) 
		{
			...
			{	if(conn!=null && !conn.getAutoCommit())
				conn.rollback();
				//e.printStackTrace() ;
				Log.info("更新发货信息失败，美丽说单号【" + orderID + "】，快递公司【" + postCompany + "】，快递单号【" + postNo + "】。错误信息：" + e.getMessage()) ;
			}
	...
	private  static boolean delivery(String jobname,Connection conn,Hashtable<String, String> hto)
	{
		...
		{	
			//方法名
			String apimethod="/order/deliver";
			HashMap<String,Object> map = new HashMap<String,Object>();
	        map.put("order_id", orderCode);
	        map.put("express_code", postCompany);
	        map.put("vcode", Params.vcode);
	        map.put("apimethod", apimethod);
	        map.put("express_id", postNo);
	        //发送请求
			String responseText = CommHelper.doPost(map,Params.url);
			Log.info("返回数据 ："+responseText);
			//把返回的数据转成json对象
			JSONObject responseObj= new JSONObject(responseText);
			int resultCode= responseObj.getInt("code");
			//错误对象 
			if(resultCode!=0){   //发生错误
				String operCode = responseObj.getString("message");
				Log.error("美丽说订单发货作业错误", "美丽说订单发货作业，错误码："+operCode);
				flag=false;
			...
			else
			{
				...
				int successNum= responseObj.getJSONObject("info").getInt("affect");
				if(successNum==1){
					flag=true;
				}
			}
			...
			Log.info("更新发货信息失败，美丽说单号【" + orderCode + "】，快递公司【" + postCompany + "】，快递单号【" + postNo + "】。错误信息：" + e.getMessage()) ;
			flag=false ;
		}
