...
public class UpdateStock extends Thread{
	...
	private static String jobname = "更新名鞋库库存作业";
	...
	public void run() {
		...
		do {		
			...
			while (System.currentTimeMillis() - startwaittime < (com.wofu.ecommerce.s.Params.waittime * 1000))		
				try {
					sleep(1000L);
				} catch (Exception e) {
	...
	private void updateStock(Connection conn) throws Exception
	{
		...
		for(int i=0;i<vtinfo.size();i++)
		{
			...
			{		
				...
				for (int j=0;j<vtstockconfigsku.size();j++)
				{
					...
					if (n>=100){
						...
						StockUtils.batchUpdateStock(dao,orgid,sku,String.valueOf(2));
			...
			}catch(Exception je)
			{
				...
				Log.error(jobname,"更新名鞋库库存失败,单号:"+tid+" SKU:" +sku+" "+je.getMessage());
			}
		}
