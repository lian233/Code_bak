...
public class CheckSfExpressNoExecuter extends Executer {
	...
	private void getsfExpressId() throws Exception {
		String sql = "select top 1 a.sheetid,a.paymode,isnull(a.payfee,0) payfee,a.linkman,a.tele,a.address, isnull(b.address,'广东省 广州市 番禺区 化龙镇石化公路明经路段9号（汉林电器旁）') j_address, isnull(b.LinkMan,'张成') j_linkman,isnull(b.MobileNo,'18939940107') j_mobile from outstock0 a,ownerdetail b  where  a.inid=b.detailID and b.chaintypeid=12 and a.delivery='sf' and a.flag>=10 and ISNULL(a.DeliverySheetID,'')=''";
		...
		for(int i=0;i<result.size();i++){
			try{
				...
				.append("custid='").append(custid).append("'> ");//月结帐号改到这里了
