	...
	{		
		try 
		{
			...
			sql =  new StringBuilder().append("insert into ns_customerorder(CustomerOrderId , SheetID , Owner , tid , OrderSheetID , sellernick , ")
                ...
                .append(String.valueOf(priceTemp)).append( "','")//11  原来o.getOrderPayment() 这里只取订单总金额-商家优惠金额
                ...
                .append(user!=null?user.getFullname().replaceAll("'","''"):"").append("','','").append(user!=null?user.getFullname().replaceAll("'","''"):"").append("','").append(user!=null?user.getProvince():"").append("','").append(user!=null?user.getCity():"").append("','").append(user!=null?user.getCounty():"").append("','")
			...
			if(itemList!=null){
				for(int i = 0 ; i < itemList.size() ; i ++)
				{
					...
					sql = "insert into ns_orderitem(CustomerOrderId , orderItemId  , SheetID , skuid , itemmealname , "   //5
	                    ...
	                    + "'" + item.getSkuName() + "','" + username + "','" + user.getFullname().replaceAll("'", "''") + "','','" + Formatter.format(o.getOrderStartTime(), Formatter.DATE_TIME_FORMAT) + "',"  //10
