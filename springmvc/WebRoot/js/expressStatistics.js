...
$(function(){
	...
	//添加行数据(客户表)
	function addGridRow(){
		var txt = "<tr> <td hidden></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td> </tr>";
		...
		$(cells[2]).html("<label></label>");	//客户
		$(cells[3]).html("<label></label>");	//快递
		$(cells[4]).html("<label></label>");	//期初余量
        $(cells[5]).html("<label></label>");	//充值数量
        $(cells[6]).html("<label></label>");	//使用数量
        $(cells[7]).html("<label></label>");	//期末余量
		return tr;
	}
...
//			field.text(record.CustomerID);
			...
			//期初余量
			field = $(label[4]);
			field.text(record.StartQty);
            //充值数量
            field = $(label[5]);
            field.text(record.InQty);
            //使用数量
            field = $(label[6]);
            field.text(record.OutQty);
            //期末余量
            field = $(label[7]);
            field.text(record.EndQty);
		}
	}
