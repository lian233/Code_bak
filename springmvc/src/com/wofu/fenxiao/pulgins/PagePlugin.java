...
public class PagePlugin implements Interceptor {
	...
	private String generatePagesSql(String sql, PageView page) {
		if (page != null) {
			if("mysql".equals(dialect)){
				...
				return buildPageSqlForSQL(sql, page).toString();
			}
		}
		return sql;
	}
    ...
    public StringBuilder buildPageSqlForMysql(String sql, PageView page) {
        ...
        String beginrow = String.valueOf((page.getPage() - 1) * page.getPsize());
        pageSql.append(sql);
        pageSql.append(" limit " + beginrow + "," + page.getPsize());
        return pageSql;
    }
     ...
     * @return String
     */
    public StringBuilder buildPageSqlForSQL(String sql, PageView page) {
        StringBuilder pageSql = new StringBuilder(150);
        String beginrow = String.valueOf((page.getPage() - 1) * page.getPsize());
        String endrow = String.valueOf(page.getPage()*page.getPsize());
        pageSql.append("select * from (select row_number() over(order by ").append(page.getId()).append(") as rowid,*")
        .append(" from (").append(sql).append(") as a) as b where rowid>").append(beginrow)
        .append(" and rowid<=").append(endrow);
        return pageSql;
    }
