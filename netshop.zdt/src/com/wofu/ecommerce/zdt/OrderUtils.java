...
public class OrderUtils {
	...
	public static String createInterOrder(Connection conn,
		...
		try {
			...
			int paymode="在线支付".equals(o.getType()) || "线下支付".equals(o.getType())?1:2;
			...
			sql = "insert into ns_customerorder"
					+ "(CustomerOrderId , SheetID , Owner , tid  , sellernick , paymode,invoiceflag,"
					...
					+ " values('"+ sheetid+ "','"+ sheetid+ "','"+username+"','"+ o.getOrder_no()
					+ "','"+ username+ "', "+paymode+","+invoiceflag+",'"+Formatter.format(o.getCreated(),Formatter.DATE_TIME_FORMAT)+"',"+ o.getPayment()+ ", '"
					+ o.getStatus()+ "' , '"+o.getBuyer_memo() + "' , '"+ o.getSeller_memo()+ "','"+Formatter.format(o.getCreated(), Formatter.DATE_TIME_FORMAT)+"',"
					...
					+ o.getReceiver_mobile()+ "' , '"+ o.getReceiver_phone()+ "','ZDT'," + tradecontactid + ")";
			SQLHelper.executeSQL(conn, sql);
			
			for (int i=0;i<o.getOrderItems().getRelationData().size();i++) {
				
				OrderItem item = (OrderItem) o.getOrderItems().getRelationData().get(i);
				...
				sql = "insert into ns_orderitem(CustomerOrderId , orderItemId  ,oid, SheetID  ,skuid, itemmealname , "
						...
						+ "'"+ sheetid+ "','"+ sheetid+ item.getOrder_item_id()+ "','"+item.getOrder_item_id()+"','"+ sheetid+ "','0','"+ item.getTitle()
			...
			Log.info("生成订单【" + o.getOrder_no() + "】接口数据成功，接口单号【"+ sheetid + "】");
			...
			throw new JException("生成订单【" + o.getOrder_no() + "】接口数据失败,错误信息："+ e1.getMessage());
		}
	}
	/**
