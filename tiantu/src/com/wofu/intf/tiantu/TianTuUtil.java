...
public class TianTuUtil
{
	...
	{
		String msgid = "";
		String sql = "select count(*) from ecs_idlist where tablename='tiantu'";
		if (SQLHelper.intSelect(connection, sql) > 0)
		{
			sql = "update ecs_idlist set keyid=keyid+1 where  tablename='tiantu'";
			SQLHelper.executeSQL(connection, sql);
		} else
		{
			sql = "insert into ecs_idlist(keyid,keyname,tablename) values(1,'msgid','tiantu')";
			SQLHelper.executeSQL(connection, sql);
		}
		sql = "select keyid from ecs_idlist where tablename='tiantu'";
	...
	public static void recordMsg(Connection connection, String msgid, String infsheetid, int infsheettype, String servicetype)
		throws Exception
	{
		String sql = "insert into ecs_tiantumsg(msgid,infsheetid,infsheettype,servicetype,reqdate) "
	...
	public static void updateMsg(Connection connection, String infsheetid, int infsheettype, int status)
		throws Exception
	{
		String sql = "update ecs_tiantumsg set replydate=getdate(),status="+status
	...
	*/
	...
	throws Exception
	{
		...
		String sql="select warehousecode from ecs_tiantuwarehousecontrast where customercode='"+customercode+"' and dcshopid='"+shopid+"'";
	...
	{
		String shopid = "";
		String sql="select dcshopid from ecs_tiantuwarehousecontrast "
	...
	{
		String dsname = "";
		String sql="select dsname from ecs_tiantuwarehousecontrast "
			+"where customercode='"+customercode+"' and warehousecode='"+warehousecode+"'";
