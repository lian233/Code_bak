			...
			try {					
				...
				/*	sql="select b.custombc,b.goodsname,a.qty,b.colorname,b.sizename "
					...
					for (int i=0;i<vtsku.size();i++)
					{
						...
						bizData.append("<itemNote>"+itemnote.trim()+"</itemNote>");				
					...
					result=result.substring(result.indexOf("<AsnInfo>"),result.indexOf("</AsnInfo>")+10);
					Log.info("result:　"+result);
					...
					if (flag.equalsIgnoreCase("FAILURE")) //失败
					{
						...
						if(errorMsg.indexOf("补货单已存在")>0){
							IntfUtils.backupIntfSheetList(conn,sheetid,Params.interfacesystem,"2227");
							
							TianTuUtil.recordMsg(conn, msgid,sheetid,2227,serviceType);
						}
