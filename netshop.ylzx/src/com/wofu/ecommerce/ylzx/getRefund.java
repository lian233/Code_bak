...
public class getRefund extends Thread {
	...
	public void run() {
		...
		do {		
			...
			while (System.currentTimeMillis() - startwaittime < (long) (com.wofu.ecommerce.ylzx.Params.waittime * 1000*5))		
				try {
					sleep(1000L);
				} catch (Exception e) {
				...
				while(true)
				{
					...
					for(int j=0;j<refundlist.length();j++)
					{
						try{
							...
							String responseData ="";//Utils.sendByPost(orderlistparams, app_secret, url);
