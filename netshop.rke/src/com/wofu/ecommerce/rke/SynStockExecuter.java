...
public class SynStockExecuter extends Executer {
	...
	private void synStock() throws Exception
	{
		...
		int n=0;
		//仓库数
		sql ="select distinct c.orgcode,b.synstockrate from ecs_tradecontactorgcontrast a,ecs_rationconfig b ,"
			+"ecs_org c where A.tradecontactid="+tradecontactid+"  and a.orgid=b.shoporgid and b.rationorgid=c.orgid ";
		Vector orgs = this.getDao().multiRowSelect(sql);
		for (int k=0;k<10;)
		{
			try 
			{
						...
						for(int j=0;j<vtstockconfigsku.size();j++)
						{
							try{
								...
								if (isfind)
								{
									...
									for(Iterator it =orgs.iterator();it.hasNext(); ){
										...
										if (ismulti)
										{
											...
											for(Iterator itmulti=multiskulist.iterator();itmulti.hasNext();)
											{
												...
												qty=StockManager.getTradeContactJxUseableStock(this.getDao().getConnection(), tempResult.get("orgcode").toString(),Float.valueOf(tempResult.get("synstockrate").toString()).floatValue(),customercode);
										...
										{
											qty=StockManager.getTradeContactJxUseableStock(this.getDao().getConnection(), tempResult.get("orgcode").toString(),Float.valueOf(tempResult.get("synstockrate").toString()).floatValue(),stockconfigsku.getSku());
										}
										if (qty<0) qty=0;
									...
									StockUtils.updateStock(url, ver, stockconfigsku, arr.toString());
								}
							}catch(Exception ex){
								if (this.getConnection() != null && !this.getConnection().getAutoCommit())
									this.getConnection().rollback();
