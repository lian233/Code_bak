...
public class OrderDelivery extends Thread {
	...
	private void delivery(Connection conn)  throws Exception
	{
		...
		for (int i = 0; i < vdeliveryorder.size(); i++) {
	        ...
	        //系统级参数设置
			deliveryParams.put("act", "set_order_status_center");
			deliveryParams.put("api_version", Params.ver);
			deliveryParams.put("endtime", Formatter.format(new Date(), Formatter.DATE_TIME_FORMAT));
			deliveryParams.put("ship_status", "1");//
			deliveryParams.put("order_status", "5");//
			deliveryParams.put("outid", post_no);//运单号
			deliveryParams.put("order_id", orderid);//订单号
			deliveryParams.put("company", post_company);//物流公司
			deliveryParams.put("delivery_goods", getDeliveryDetail(conn,orderid));//
	        
			String responseDeliveryData = Utils.sendByPost(deliveryParams,Params.url);
			Log.info("responseDeliveryData: "+responseDeliveryData);
	        
			Document doc = DOMHelper.newDocument(responseDeliveryData,"gbk");
			Element ele = doc.getDocumentElement();		
			String errorCode=DOMHelper.getSubElementVauleByName(ele, "result");
