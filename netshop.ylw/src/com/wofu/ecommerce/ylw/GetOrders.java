...
public class GetOrders extends Thread {
	...
	private static String jobname = "获取邮乐网订单作业";
			...
			try 
			{
				...
				while(hasNextPage)
				{
					...
					if(totalInfo==null){
						...
						Log.error("获取邮乐网订单列表", "获取订单列表失败，操作码："+operCode);
						hasNextPage = false ;
						break ;
					}
						...
						{	
							...
							if("0".equals(o.getReturnOrderFlag())){   //正常订单
								...
								else if("40".equals(o.getOrderLineStatus()))
								{
									...
									//取消订单失败
									if(resultCode == 2)			
										Log.info("邮乐网请求取消订单失败,单号:"+orderCode+"");						
									else
										Log.info("邮乐网请求取消订单成功,单号:"+orderCode+"");
